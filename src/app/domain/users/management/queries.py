...
from app.domain.users.core.schemas import (User, UserDetailsFull,
                                           UserListRequestSchema, UserWhere)
...
class UserManagementListQuery:
    ...
    async def __call__(self, payload: UserListRequestSchema) -> Paginated[User]:
        ...
        if payload.where is None:
            payload.where = UserWhere(permissions="student")
        else:
            payload.where.permissions = "student"
