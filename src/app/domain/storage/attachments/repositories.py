...
class AttachmentRepository:
    ...
    async def get_attachments(
        ...
        async with self.transaction.use() as db:
            if pagination is None:
                ...
                return NoPaginationResults([schemas.Attachment.model_validate(x) for x in results])

            return await pagination(db, query)

    async def get_attachment_or_none(self, attachment_id: UUID) -> schemas.Attachment | None:
    ...
    async def create_attachment(self, payload: schemas.AttachmentCreateFull) -> IdContainer:
        query = insert(models.Attachment).values(**payload.model_dump()).returning(models.Attachment.id)
